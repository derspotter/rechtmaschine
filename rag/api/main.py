...
class RagDeleteRequest(BaseModel):
    chunk_ids: Optional[list[str]] = Field(default=None, max_length=1024)
...
def _select_rerank_candidates(
    ...
    """Cap how many fused candidates go to the cross-encoder reranker.

    The cross-encoder scores every (query, text) pair on the GPU, so sending all
    of dense_top_k + sparse_top_k candidates is slow. Plain fused-score top-N is
    enough at this corpus size (~9k chunks); revisit with per-retriever
    guaranteed slots only if reranked retrieval misses exact statute matches.
    """
...
@app.post("/v1/rag/chunks/delete")
def delete_chunks(body: RagDeleteRequest, x_api_key: Optional[str] = Header(default=None)) -> dict[str, Any]:
    _validate_api_key(x_api_key)
    if not body.chunk_ids and not body.collection:
        raise _error(
            "invalid_delete_request",
            "Provide chunk_ids, a collection, or both",
            422,
        )
    ...
    if body.chunk_ids:
        ...
        params.append(body.chunk_ids)

    with _db_conn() as conn, conn.cursor() as cur:
        cur.execute(f"DELETE FROM rag_chunks WHERE {' AND '.join(clauses)}", params)
...
def retrieve(body: RagRetrieveRequest, x_api_key: Optional[str] = Header(default=None)) -> dict[str, Any]:
    ...
    if body.use_reranker:
        candidates = _select_rerank_candidates(candidates)
